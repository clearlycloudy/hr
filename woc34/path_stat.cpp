...
int main() {
    ...
    //calculate shortest paths between group clusters
    ...
    for(int i = 0; i < q; ++i ){
        ...
        while(group_current!=group_v){
            ...
            while(!s.empty()){
                ...
                s.pop_front();
                for( auto neigh : ns[c].nb ){
                    if(visited[neigh] == false && group[neigh] == group[c]){
                        visited[neigh] = true;
                        prev[neigh] = c;
                        if(neigh==node_u_dest){
                            found = true;
                            break;
                        }
                        s.push_back(neigh);
